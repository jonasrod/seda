...
if(!isset($_SESSION['brw_idLogin'])) {
	$_SESSION['fechamento']['redirecionamento'] = 'rastreamento.php';
...
$sql = "select * from venda where status = 1 and mClienteId = " . $_SESSION['brw_idLogin'] . " order by dtCadastro asc";
...
<div id="maincontainer">
    ...
    <div class="container">
      ...
      <h1 class="productname">Rastreamento</h1>
      <p>Utilize o c&oacute;digo de rastreamento no site dos <a href="http://www2.correios.com.br/sistemas/rastreamento/" target="_blank">Correios</a></p>
      <div class="cart-info">
        <table class="table table-striped table-bordered">
          <tbody><tr>
            <th width="14%" class="image">Data da Compra</th>
            <th width="21%" class="name">N&uacute;mero do Pedido</th>
            <th width="14%" class="model">Código de Rastreamento</th>
          </tr>
          <?php 
          foreach( $objVenda->listarComFiltro( $result ) as $venda ) { ?>
	          ...
	          <tr>
	            ...
	            <td class="name"><?php echo $venda->getReferencia();?></td>
	            <td class="model">
	            	<?php 
	            	if ($venda->getRastreamento() == '') {
	            		echo 'Em processamento...';
	            	} else {
	            		echo "<a href='http://websro.correios.com.br/sro_bin/txect01$.QueryList?P_LINGUA=001&P_TIPO=001&P_COD_UNI=".$venda->getRastreamento()."' target='_blank'>";
	            		echo $venda->getRastreamento() . "</a>";
	            	}
	            	?>
	            </td>
	          </tr>
