...
include_once '../../wcm/model/classesGeradas/model/endereco.class.php';
include_once '../model/clienteid.class.php';
include_once '../model/tipoenderecoid.class.php';
include_once '../model/estadoid.class.php';
...
<div class="control-group">
	<label for="estadoid" class="control-label span3">EstadoID</label>
	<select name="estadoid" id="estadoid" class="span9" required="true">
	<option value="" disabled="disabled" selected="selected">Selecione a estadoid</option>
	<?php foreach( $objEstadoID->listarEstadoID() as $estadoid ) { ?>
		<option value="<?=$estadoid->getEstadoIDID()?>" <?php if( $estadoid->getEstadoIDID() == $objEndereco->getEstadoID()->getEstadoIDID() ){ ?>selected="selected"<?php } ?>>
		<?=$estadoid->getDescricao()?></option>
	<?php } ?>
	</select>
</div>
