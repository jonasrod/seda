...
class Endereco
{
	...
	public function obterEnderecoFPorCliente( $clienteID )
	{
		$sql = "select * from endereco where mClienteId = $clienteID and mTipoEnderecoId = 1"; // seleciona o endereco de entrega
		$result = $this->bd->executarSQL( $sql );
		return $this->montarObjeto( $result->fetch_array() );
	}
	...
	//		$objEstadoID = new Estado();
	//		$objEstadoID->obterEstado( $row["mEstadoID"] );
